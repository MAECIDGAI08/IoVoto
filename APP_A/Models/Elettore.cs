    ...
    public class Elettore
    {
        ...
        public static Elettore parseElettoreFromJSON(String JSON)
        {
            ...
            if (resultError.Value != null)
            {
                ...
                // Condizione se il dato ricevuto da Spid non viene trovato in BC
				if (payload.Count() == 0) {
                    //Entro se i parametri non sono presenti nella lista elettori 
                    //Entro se uno dei due dati che vengono passati risulta errato 
                    errors.ErrorDescription = "No record found";
                    return errors;
                }
                // Condizione se viene ricevuto un dato duplicato
                if (payload.Count() > 1)
                {
                    errors.ErrorDescription = "Duplicate Record in BlockChain";
                    return errors;
                }
